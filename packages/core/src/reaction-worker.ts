...
// Option-level constraints must live in this outer refinement: a discriminated union
// only accepts plain object options, so wrapping one in `.refine` breaks the schema.
export const ReactionActionSchema = z
  .discriminatedUnion("type", [
    z.object({ type: z.literal("github.comment"), content: ExactText }),
    z.object({
      type: z.literal("github.labels"),
      add: z.array(ExactText).default([]),
      remove: z.array(ExactText).default([]),
    }),
    z.object({ type: z.literal("linear.comment"), content: ExactText }),
    z.object({ type: z.literal("linear.transition"), stateId: ExactText }),
    z.object({
      type: z.literal("linear.priority"),
      priority: z.enum(["urgent", "high", "medium", "low", "none"]),
    }),
  ])
  .superRefine((value, ctx) => {
    if (value.type === "github.labels" && value.add.length + value.remove.length === 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "must change a label" });
    }
  });
...
export type Reservation =
  { allowed: true; businessId: string } | { allowed: false; reason: string };
...
function normalizedAction(action: ReactionAction): ReactionAction {
  ...
  if (add.some((label) => remove.includes(label)))
    throw new Error("label cannot be both added and removed");
...
export function reactionHash(input: Omit<ReactionEnvelope, "hash">): string {
  const parsed = ReactionEnvelopeSchema.omit({ hash: true }).parse({
    ...input,
...
export function createReactionEnvelope(input: Omit<ReactionEnvelope, "hash">): ReactionEnvelope {
  const normalized = {
    ...input,
    action: normalizedAction(ReactionActionSchema.parse(input.action)),
  };
...
export function reserveReaction(input: {
  ...
  if (
    !approval.active ||
    approval.used ||
    approval.hash !== envelope.hash ||
    approval.approvalId !== envelope.approvalId ||
    approval.approvalVersion !== envelope.approvalVersion ||
    approval.approverId !== envelope.approverId
  )
  ...
  if (
    authority.policyVersion !== envelope.policyVersion ||
    !authority.allowedResources.includes(envelope.resource) ||
    !authority.allowedActionTypes.includes(envelope.action.type) ||
    !authority.credentialActive
  )
    return { allowed: false, reason: "policy_or_scope_denied" };
  if ((envelope.provider === "github") !== envelope.action.type.startsWith("github."))
    return { allowed: false, reason: "provider_action_mismatch" };
  if (
    envelope.action.type === "linear.transition" &&
    !authority.linearStateIds?.includes(envelope.action.stateId)
  )
    return { allowed: false, reason: "linear_state_denied" };
  if (
    envelope.action.type === "github.labels" &&
    [...envelope.action.add, ...envelope.action.remove].some(
      (label) => !authority.githubLabels?.includes(label),
    )
  )
    return { allowed: false, reason: "github_label_denied" };
  if (!authority.budgetAvailable) return { allowed: false, reason: "budget_exhausted" };
  if (!authority.resourceSlotsAvailable || !authority.workspaceSlotsAvailable)
    return { allowed: false, reason: "concurrency_exhausted" };
