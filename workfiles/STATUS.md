...
The Cloudflare-native hosted path now has an isolated preview Worker plus provisioned D1 control/event databases, private R2 payload storage, an ingestion Queue and DLQ, and applied initial migrations. Its signed canary ingress implements encrypted payload storage, D1 idempotency/outbox/audit state, Queue publication, duplicate recovery, and scheduled outbox repair. A deployed signed canary returned `202` and reconciled to one processed event, one published outbox item, and one audit entry. Preview ingress was gated again immediately after acceptance; all authenticated `/v1` APIs remain deliberately gated until Better Auth and tenant repositories are implemented.
