...
Phase A foundation: a single deterministic evaluator now emits policy/context digests, evaluator/schema provenance, outcome, scope, matched rules and reason codes. Pack manifests are immutable/content-addressed and `009_policy_packs.sql` provides tenant-composite version, serialized activation, and simulation records. Phase B foundation: simulation invokes that same evaluator only and accepts evidence inputs as data; it has no action, approval, incident, billing, reaction, notification, or mutation adapter. Missing retained evidence and authorization produce blocked/partial coverage rather than a complete claim.
