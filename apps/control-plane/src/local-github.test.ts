...
describe("local GitHub webhook bootstrap", () => {
  ...
  it("uses GitHub's signed JSON webhook configuration", () => {
    expect(webhookFormArgs("https://eventforge.trycloudflare.com", "secret")).toEqual(
  ...
  it("finds the temporary Cloudflare hostname in either cloudflared output stream", () => {
    expect(quickTunnelUrl("INF | https://eventforge-sky.trycloudflare.com | connected")).toBe(
      "https://eventforge-sky.trycloudflare.com",
    );
    expect(
      quickTunnelUrl("INF Cloudflare API https://api.trycloudflare.com is reachable"),
    ).toBeUndefined();
    expect(quickTunnelUrl("no public URL yet")).toBeUndefined();
  });

  it("keeps the webhook path when Cloudflare forwards the local origin", () => {
    expect(publicWebhookUrl("https://eventforge-sky.trycloudflare.com")).toBe(
      "https://eventforge-sky.trycloudflare.com/webhooks/github",
    );
  });

  it("isolates Quick Tunnels from an unrelated default cloudflared config", () => {
    expect(quickTunnelArgs("http://127.0.0.1:4310", "/dev/null")).toEqual([
      "--config",
      "/dev/null",
      "tunnel",
      "--url",
  ...
  it("keeps relay state out of the repository root", () => {
    expect(localWebhookStatePath("/tmp/eventforge")).toBe(
      "/tmp/eventforge/.eventforge/github-local-webhook.json",
    );
  });
});
